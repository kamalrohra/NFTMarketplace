...
app.post("/Q1", async (req, res) => {
  ...
  var { address1, address2 } = req.body;
...
app.post("/Q2", async (req, res) => {
  ...
  try {
    ...
    res.send({ data: arr2, eths, countMinted, price });
  } catch (err) {
    console.log(err);
    console.log(err.message);
...
app.post("/Q5", async (req, res) => {
  ...
  // var address = "0x614917F589593189ac27aC8B81064CBe450C35e3";
  const data1 = await fetchDataContract(address);
  var arr1 = data1.data.result;
  let price = [];
  let Minted = [];
  let countMinted = 0;
